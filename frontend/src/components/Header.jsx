      ...
      <div className="container" style={{ padding:'.7rem 1rem', display:'flex', alignItems:'center', justifyContent:'space-between', gap:'.8rem', flexWrap:'wrap' }}>
        <Link to="/" style={{ textDecoration:'none', color:'inherit' }}>
          <div style={{ display:'flex', alignItems:'baseline', gap:'.4rem' }}>
            ...
            <span style={{ fontFamily:'var(--font-mono)', fontSize:'.6rem', color:'var(--grey)', letterSpacing:'.08em' }}>v2.0</span>
          </div>
        ...
        <div style={{ display:'flex', gap:'.4rem', alignItems:'center', flexWrap:'wrap' }}>
          ...
          {nav.map(n => (
            <Link key={n.to} to={n.to} style={{
              fontFamily:'var(--font-display)', fontSize:'.9rem', letterSpacing:'.05em',
              color: loc.pathname===n.to ? 'var(--paper)' : 'var(--ink)',
              background: loc.pathname===n.to ? 'var(--ink)' : 'transparent',
              border:'2px solid var(--ink)', padding:'.28rem .85rem',
